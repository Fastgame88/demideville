(async()=>{
  ...
  const fitPage=()=>{if(window.innerWidth<=900){document.body.style.removeProperty('--gallery-header-scale-x');document.body.style.removeProperty('--gallery-header-scale-y');document.body.style.removeProperty('--product-scale-x');document.body.style.removeProperty('--product-scale-y');syncProductHeight();return}/* Keep the complete product layout proportional when the desktop window height changes. The old separate X/Y scales compressed the image vertically. */const scale=Math.max(.01,window.innerWidth/1920);document.body.style.setProperty('--gallery-header-scale-x',String(scale));document.body.style.setProperty('--gallery-header-scale-y',String(scale));document.body.style.setProperty('--product-scale-x',String(scale));document.body.style.setProperty('--product-scale-y',String(scale));requestAnimationFrame(syncProductHeight)};
  let fitFrame=0;
