(async()=>{
  ...
  const fitPage=()=>{if(window.innerWidth<=900){document.body.style.removeProperty('--gallery-header-scale-x');document.body.style.removeProperty('--gallery-header-scale-y');document.body.style.removeProperty('--product-scale-x');document.body.style.removeProperty('--product-scale-y');document.body.style.removeProperty('--product-inverse-scale');document.body.style.removeProperty('--product-visual-left');document.body.style.removeProperty('--product-visual-top');syncProductHeight();return}/* Keep the master layout responsive, but counter-scale the actual product media/copy so their physical size never changes with the browser width. Keep the 543×643 media box centered in the left half of the visible window instead of letting the scaled master push it down/right. */const scale=Math.max(.01,window.innerWidth/1920),inverse=1/scale,mediaW=543,mediaH=643;const visualLeftPx=Math.max(24,window.innerWidth*.25-mediaW/2),visualTopPx=Math.max(110,(window.innerHeight-mediaH)/2);document.body.style.setProperty('--gallery-header-scale-x',String(scale));document.body.style.setProperty('--gallery-header-scale-y',String(scale));document.body.style.setProperty('--product-scale-x',String(scale));document.body.style.setProperty('--product-scale-y',String(scale));document.body.style.setProperty('--product-inverse-scale',String(inverse));document.body.style.setProperty('--product-visual-left',`${visualLeftPx/scale}px`);document.body.style.setProperty('--product-visual-top',`${visualTopPx/scale}px`);requestAnimationFrame(syncProductHeight)};
  let fitFrame=0;
